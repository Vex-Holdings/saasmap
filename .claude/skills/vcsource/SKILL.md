...
Per-source notes:
- a16z (`a16z.com/portfolio/?status=Active`, structure as of 2026-08): no DOM scraping needed — the page embeds its full portfolio dataset as a global `window.a16z_portfolio_companies` (~855 entries) with `title` and `web` fields; read it via one browser_evaluate. "Active" = entries where `display_exit_info_on_a16z_website` is false (~658). The `?status=Active` URL param only affects client-side rendering, not the embedded array. Expect hundreds of entries.
- Sequoia (`sequoiacap.com/our-companies/#all-panel`, structure as of 2026-08): the "all" panel renders only ~104 rows and shows no websites — don't scrape it. The site is WordPress VIP: `GET /wp-json/wp/v2/company?per_page=100&page=N&_fields=title,slug` returns all companies (~419); each `/companies/<slug>/` detail page carries the website as the first `<a href>` inside `<div class="company__brand"><h1>` (the h1 may or may not have a class — match `<h1[^>]*><a href="...">`). Companies with no anchor there are defunct — treat as name-only. Expect hundreds of entries.
- Greylock (`greylock.com/portfolio/`, structure as of 2026-08): Next.js table, not a card grid. Company rows are `li` elements with class `border-b`; the company name is the row's `img[alt]` (strip a trailing " logo" and collapse whitespace). The listing page has NO external company links — websites live on each `/portfolio/<slug>/` detail page in a `<script type="application/ld+json">` Organization block; filter for the block whose `@id` contains `/portfolio/` (each page has two Organization blocks; the other one returns greylock.com for every company). Fetching detail pages same-origin via `fetch()` inside browser JS with modest concurrency takes ~30s for ~158 companies. Expect ~150+ entries; defunct companies (e.g. Houseparty, Neeva) have no URL — treat as name-only.
